...
function getYoutubeMusicLinksFromEnv() {
  const raw = process.env.YOUTUBE_MUSIC_RELEASE_LINKS;
  ...
  try {
    ...
    throw new Error(
      `Invalid YOUTUBE_MUSIC_RELEASE_LINKS JSON. ${
...
async function main() {
  ...
  const releaseLinkConfig = getYoutubeMusicLinksFromEnv();
  const configEntries = Object.entries(releaseLinkConfig);

  if (!configEntries.length) {
    throw new Error("No YouTube Music release links found. Add YOUTUBE_MUSIC_RELEASE_LINKS.");
  }
  ...
  for (const [key, url] of configEntries) {
    ...
    releaseLinkRows.push({
      id: stableUuid("youtube-music-release-link", url),
